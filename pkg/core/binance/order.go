...
func (me *Binance) CreateOrder(symbol, buyWith, side string, quantity, price float64) error {
	_, err := me.Client.NewCreateOrderService().Symbol(fmt.Sprintf("%s%s", symbol, buyWith)).
